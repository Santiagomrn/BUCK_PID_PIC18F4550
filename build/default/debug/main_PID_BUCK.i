...
void timer_ms(unsigned int n)
{
    ...
    for(i=0;i<n;i++)
    {
        TMR0 = 65535 - (48000000UL/(4*8*1000));
        while(!TMR0IF);
        TMR0IF=0;
    }
    T0CON = 0x00;
...
void main()
{
    ...
    while(1){
        ...
        PWM(60);
        char str[20];
        sprintf(str, "%d", Control );
        LCD_String_xy(1,0,str);

        int PProporcional =(int)Proporcional;
        sprintf(str, "%d", PProporcional );
        LCD_String_xy(1,4,str);

        int PIntegral=(int)Integral;
        sprintf(str, "%d", PIntegral );
        LCD_String_xy(1,7,str);

        int PDerivativo=(int)Derivativo;
        sprintf(str, "%d", PDerivativo);
        LCD_String_xy(2,4,str);

        int pE=(int)Error;
        sprintf(str, "%d", pE);
        LCD_String_xy(2,0,str);

        int pinput=(int)input;
        sprintf(str, "%d", input);
